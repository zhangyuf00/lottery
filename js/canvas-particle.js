var CanvasParticle = (function(){
	...
	function canvasInit(canvasConfig){
		...
		canvas.element.style = "position: fixed; top: 0; left: 0; z-index: -1;background-color:#000000;background-image: url(./img/bg.jpg);background-size: 100% 100%;background-position: center center;background-repeat: no-repeat;";
			...
			for(var j = 0; j < len; j++){
				if(i != j){
					...
					if(dist <= canvas.config.dist && canvas.points[i].max_conn <canvas.config.max_conn){
						...
						// 距离越远，线条越细，而且越透明
						context.lineWidth = 0.7 - dist / canvas.config.dist;
